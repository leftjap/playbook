...
| B-22 | 운동 | 운동 화면 UX 개편 Phase 1-B~4-C | P1 | ✅ 완료 (Phase 3-A 헤더 개편) |
...
- 3/26: ✅ B-22 Phase 3-A 헤더 개편. 타이머(좌) + 볼륨 비교(우) 1행 레이아웃. updateHeaderVolume() 함수로 현재/지난번 세션 볼륨 비교. 부위 탭 제거, 부위 필터(_headerFilterPart) 항상 null.
- 3/26: ✅ B-22 Phase 2-B 운동 중 종목 삭제 완료. 종목 헤더 롱프레스 → 액션시트 "종목 완료"/"종목 삭제" 선택. 삭제 시 세션 제거 + 부위 태그 재계산 + 인덱스 보정 + 타이머 정리.
- 3/26: B-22 Phase 2-A 완료. 운동 중 종목 추가 — 네비 바 우측 "+" 버튼 → 액션시트로 부위별 종목 선택 → 세션에 추가.
