...
Sentry MCP(stdio) 연동됨. org: leftjap.
디버깅 작업지시서에 도구명(search_issues, get_issue_details, analyze_issue_with_seer)과 organizationSlug, projectSlug을 명시할 것.
...
| 운동 | `C:\dev\apps\gym\` | `leftjap/gym` | 루트 | MVP 실사용 중, 버그 다수, Sentry 도입 완료, MCP(stdio) 연동 |
| 어학 | `C:\dev\apps\study\` | `leftjap/study` | 루트 | Phase 2 중간, Sentry 도입 완료 |
| 크립토 | `C:\dev\docs\투자 전략\` | `leftjap/docs` | `투자 전략/AGENTS.md` | 문서 수시 갱신 |
| keep | `C:\dev\apps\keep\` | `leftjap/keep` | 루트 | B-55 검증 대기, 로컬 일일 백업 운영 중, Sentry 에러 모니터링 도입 완료, MCP(stdio) 연동, 보안 조치 완료(Gemini+CSE 키 삭제, Code.js 데드 코드 제거, 2026-03-30) |
...
| ID | 항목 | 완료일 | 상태 |
|---|---|---|---|
| B-56 | keep 에디터 제목 입력 시 리스트 미반영 | 3/30 | ✅ 완료 |
| B-57 | keep iOS PWA 제목→본문 탭 시 UI 깨짐 | 3/30 | ✅ 완료 |
| B-61 | keep iOS PWA 초기 로드 시 빈 문서 자동 생성 방지 | 3/30 | ✅ 완료 |

### B-61 keep iOS PWA 초기 로드 시 빈 문서 자동 생성 방지
...
- **원인**: `app.js` init() 함수에서 `getDocs('navi')` 반환이 빈 배열이고 `SYNC._dbLoading = false`일 때 `newDoc()`을 무조건 호출하는 로직이 있었음. 서버 응답이 미완료 상태와 구분되지 않아 빈 문서가 생성됨.
- **해결**: `js/sync.js`에 `_serverLoaded: false` 플래그 추가 → `loadDatabase()` 성공 시에만 true로 설정. `js/app.js` init()에 조건 추가: `SYNC._dbLoading || (GAS_URL && !SYNC._serverLoaded)` 상태일 때는 빈 문서 생성 보류.
- **관련 코드**: js/sync.js (_serverLoaded), js/app.js (init), js/data.js, js/sync.js (loadDatabase)
- **참고 문서**: keep/docs/data-safety-report.md
- **커밋**: `234839b` (B-61 iOS PWA 빈 문서 자동 생성 방지)
- **현재**: ✅ 완료 (3/30)
