...
| B-10 | 운동 | 뒤로가기 시 버그 (화면 전환 오류) | 완료 | 스와이프 뒤로가기 4건 수정: iOS 네이티브 에지 스와이프 경합 해소, 설정→운동 peek 화면 분기, 하단 버튼 safe-area 수정, 빠른 스와이프 시 cleanup이 main-view를 none으로 되돌리는 문제 수정 |
| B-11 | 운동 | finishWorkout에 확인 모달 추가 (현재 즉시 종료) | 완료 | FINISH WORKOUT 버튼 시 완료 세트 수 표시 + 확인/취소 선택 |
| B-12 | 운동 | 운동 종목 선택 방식 변경 | 완료 | 종목 부위 이동 구현: 롱프레스 드래그 → 부위 탭 드롭. 기본 종목은 partOverride 맵, 커스텀 종목은 bodyPart 직접 변경. |
...
| B-22 | keep | 파트너 글에 달린 댓글이 표시되지 않는 버그 | P0 | 기능 자체가 안 보이는 상태 |
...
- 3/26: ✅ B-28 Phase 3-A 헤더 개편. 타이머(좌) + 볼륨 비교(우) 1행 레이아웃. updateHeaderVolume() 함수로 현재/지난번 세션 볼륨 비교. 부위 탭 제거, 부위 필터(_headerFilterPart) 항상 null.
- 3/26: ✅ B-28 Phase 2-B 운동 중 종목 삭제 완료. 종목 헤더 롱프레스 → 액션시트 "종목 완료"/"종목 삭제" 선택. 삭제 시 세션 제거 + 부위 태그 재계산 + 인덱스 보정 + 타이머 정리.
- 3/26: B-28 Phase 2-A 완료. 운동 중 종목 추가 — 네비 바 우측 "+" 버튼 → 액션시트로 부위별 종목 선택 → 세션에 추가.
- 3/26: B-28 종목 선택 UI 수정 — 상단 패딩 홈과 일치(safe-area+12px+20px), hidden 종목 자동 제거, 선택 영역 flex 고정+구분선, 칩 색상 검은배경 유지.
- 3/26: B-28 Phase 1-A 완료. 종목 선택 화면(칩 UI + 하단 선택 영역). 부위 순서 변경(등→가슴→어깨→하체→팔→기타). 홈과 동일 상단 패딩/더보기 버튼.
